...
def main():
    ...
    shell_parser = subparsers.add_parser(
        "shell",
        help="Manage the yamd3s shell environment",
        parents=[parent_parser],
    )
    ...
    shell_parser.add_argument(
        ...
        "--list",
        action="store_true",
        help="List generated modules and exit",
    )

    shell_subparsers = shell_parser.add_subparsers(dest="shell_action")
    
    # Inherit parent_parser across shell subparsers so flags can go at the very end
    shell_subparsers.add_parser(
        "build", help="Run CMake build for modules", parents=[parent_parser]
    )
    shell_subparsers.add_parser(
        "run", help="Run the yamd3s shell", parents=[parent_parser]
    )
    shell_subparsers.add_parser(
        "refresh", help="Refresh/reload the yamd3s shell", parents=[parent_parser]
    )
    shell_subparsers.add_parser(
        "kill", help="Kill quickshell instances", parents=[parent_parser]
    )
    shell_subparsers.add_parser(
        "config",
        help="Open quickshell configuration directory in $EDITOR / Neovim",
        parents=[parent_parser],
    )

    ipc_parser = shell_subparsers.add_parser(
    ...
    ipc_parser.add_argument(
        ...
        "--list",
        action="store_true",
        help="List active IPC targets and methods",
    )
    ipc_parser.add_argument(
        "method", nargs="?", help="Method/action to trigger inside IpcHandler"
    )
    ipc_parser.add_argument(
        "--target", default="yamd3s", help="IPC target name (default: yamd3s)"
    )
    ipc_parser.add_argument(
        "args", nargs="*", help="Optional arguments for the IPC method"
    )

    # 3. Picker command group
    ...
    picker_parser.add_argument(
        "-f",
        "--format",
        choices=["hex", "rgb", "hsl", "hsv", "cmyk"],
        default="hex",
        help="Color format output",
    )
    picker_parser.add_argument(
        "-n",
        "--notify",
        action="store_true",
        help="Send desktop notification upon picking",
    )
    ...
    if args.command == "shell":
        ...
        if args.shell_action == "build":
            ...
            handle_run(
                DEFAULT_CONFIG_DIR, args.config, args.verbose, args.quiet
            )
